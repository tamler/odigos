        ...
        <div className="space-y-3 pt-2 border-t border-border/40">
          {plugin.config_keys.map((ck) => (
            <div key={ck.key} className="space-y-1">
              ...
              {ck.type === 'boolean' ? (
                <select
                  className="w-full px-3 py-1.5 rounded-md border border-border/40 bg-muted/50 text-sm"
                  value={values[ck.key] || ''}
...
export default function PluginsTab() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-6 space-y-5">
      ...
      {plugins.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">No plugins found.</p>
      )}
